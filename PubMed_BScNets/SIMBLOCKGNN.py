...
class BlockNet(torch.nn.Module):
    def __init__(self,data,num_features,num_classes,boundary_matrics,dimension=32):
        super(BlockNet, self).__init__()
        self.conv1 = GCNConv(num_features, 128, cached=True)
        self.conv2 = GCNConv(128, 32, cached=True)
        ...
        self.weights_L_0 = nn.Parameter(torch.FloatTensor(int(boundary_matrix_size), 64))
        self.weights_L_1 = nn.Parameter(torch.FloatTensor(int(boundary_matrix_size), 64))
        # reset parameters
    ...
    def g_encode(self,data):
        ...
        random_edge_num = int(edge_index.size(1) * 0.5)
        ...
        x = F.relu(self.conv1(x, sample_data_edge_index))
        x = F.dropout(x, p=0.5, training=self.training)
        x = F.relu(self.conv2(x, sample_data_edge_index))
        return x
